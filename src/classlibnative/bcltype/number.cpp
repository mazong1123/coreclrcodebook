...
FCIMPL3_VII(Object*, COMNumber::FormatDouble, double value, StringObject* formatUNSAFE, NumberFormatInfo* numfmtUNSAFE)
{
    ...
    case 'R':
        ...
        // Basically, it tries to convert double to string in 15 digits precision, and try to parse the string back to double 
        // and check if it's the same as the original input. If yes, it ends up with 15 digits precision. 
        // Otherwise, try a 17 digits precision converting and return that converted string.
        //
        // Sometimes it's not good enough to convert the double to string in 15 digits precision for round-tripping. 
        // Here is a known issue discussed here https://github.com/dotnet/coreclr/issues/10651#issuecomment-315667429. 

        // 1. Try to convert the double value to number struct in 15 digits precision.
        DoubleToNumber(value, DOUBLE_PRECISION, &number);

        // If it's NaN just return.
        if (number.scale == (int) SCALE_NAN) {
            gc.refRetVal = gc.numfmt->sNaN;
            goto lExit;
        }

        // If it's infinity just return. 
